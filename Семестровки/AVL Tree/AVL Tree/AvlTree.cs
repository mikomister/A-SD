...
namespace AVLTree
{
...

    {
        ...
        public void Insert<K>(K collection) 
            where K:IEnumerable
        {
        ...
        {
            AvlNode<T> current = Head; // помещаем текущий элемент в корень дерева
            ...
            if (Head != null) // существует ли корень дерева
            {
                ...
                while (stack.Count > 0)
                {
                    ...
                    if (current.Right != null)
                    {
                        ...
                        // Единажды перемещаемся вправо, после чего опять идем влево. 
